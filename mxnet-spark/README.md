...
[Amazon EMR](https://aws.amazon.com/emr/) along with Apache Spark simplifies the task of cluster and distributed job management. Apache Spark is a cluster computing framework that enables variety of data processing applications. Spark also efficiently partitions data across the cluster to parallelize processing. Spark tightly integrates with the [Hadoop](http://hadoop.apache.org/) ecosystem and several other big data solutions.
...
We will use the Amazon EMR to create a cluster with Spark and MXNet which you can install as applications using [EMR 5.10.0 - fix my link](https://aws.amazon.com/emr/) We will use [AWS CLI](https://aws.amazon.com/cli/) to create the cluster of **4 CORE instances of c4.8xlarge type and 1 master of m3.xlarge type**, you can also create through the EMR console.
...
--bootstrap-actions Name='install-pillow-boto3-upgrade-mxnet' Path=s3://aws-dl-emr-bootstrap/mxnet-spark-demo-bootstrap.sh \
--region <AWS-REGION> \
...
![pipeline.png](assets/pipeline.png =849x112)

Spark by default creates one task per core on the executor. Since MXNet has built-in parallelism to efficiently use all the CPU cores, we'll configure our application to create only task per executor. 
In the below code we will by configuration key `spark.executor.cores` to `1`, we will pass this conf object when creating SparkContext. When submitting the application, you'll see that we are going to set the number of executors to the number of workers available on the cluster, there by forcing 1 executor per node.
...
The batch size as determined by `args['batch']` is the number of images that can be fetched, preprocessed and run inference on each executor at once. This is bound by how much memory is available for each task.
...
def predict(img, args):
    """
    Run predication img and return the top_5 probability along with their classes
    """
    import mxnet as mx
    import numpy as np
...
### Collecting predictions.
Finally, we will collect the predictions of generated for each partitions using Spark `collect` action and write the predictions to an S3 Key. The Key can be passed as an argument to the `infer_main.py`
...
![Execution Time for inferencing 10K CIFAR-10 images](assets/execution.png =800x600)
...
![CPU Utilization.png](assets/cpu_utilization.png =800x600)
...
2) [Apache MXNet](http://mxnet.apache.org/)
3) [MXNet Symbol - Neural network graphs and auto-differentiation](http://mxnet.incubator.apache.org/tutorials/basic/symbol.html)
4) [MXNet Module - Neural network training and inference](http://mxnet.incubator.apache.org/tutorials/basic/module.html)
5) [MXNet - Using pre-trained models](http://mxnet.incubator.apache.org/tutorials/python/predict_image.html)
6) [Spark Cluster Overview](https://spark.apache.org/docs/2.2.0/cluster-overview.html)
7) [Submitting Spark Applications](https://spark.apache.org/docs/2.2.0/submitting-applications.html)
8) [Top 5 Mistakes to Avoid When Writing Apache Spark Applications](https://www.slideshare.net/cloudera/top-5-mistakes-to-avoid-when-writing-apache-spark-applications)
